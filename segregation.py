...
trial2 = 0 #if including "Nordic" in the majority group
trial3 = 0 #if including "Western Europe" in the majority group
...
def evenness(sample,tot,num):
    ...
    for i in range(1,num):
        numer = (abs((sample[i]/tot[i]) - P))*tot[i]
        den = 2*P*tot[0]*(1-P)
        frac = numer/den
        if sample[i]!=-1:
            D = D+frac
...
#--FEB 27 SECOND VERSION: Calculate centralization --
def centralization(geoArr,sample,num,idx,tot):
    A = 0; B = 0
    allArea = sum(geoArr[0:num-1]) #total land area considered
    geoArr = np.copy(geoArr[0:num-1]); geoArr = geoArr[idx] #remove Marka and sort area by distance to CBD
    sample = np.copy(sample[1:num-1]); sample = sample[idx] #sort minority population by distance
    tot = np.copy(tot[1:num-1]); tot = tot[idx] #sort total population by distance
    cumArea = 0; cumPop = sample[0]/sum(sample)
    for i in range(1,num-2): #from the second until last
        cumArea = cumArea+(geoArr[i]/allArea) #cumulative area for current tract
        A = A+cumArea*cumPop #update summand
        cumPop = cumPop+(sample[i]/sum(sample)) #cumulative area for current tract (not seen until next iteration)
    cumArea = geoArr[0]/allArea; cumPop = 0
    for i in range(1,num-2):
        cumPop = cumPop+(sample[i]/sum(sample))
        B = B+cumArea*cumPop
        cumArea = cumArea+(geoArr[i]/allArea)
    return(A-B)
#----------------------------------------------------
...
#--FEB 27 SECOND VERSION: Calculate concentration--
def concentration(num,sample,idx,geoArr,maj,tot):
    A=0; B=0; C=0;D=0;n1=0;n2=0
    sample = np.copy(sample[1:num-1]); sample=sample[idx]
    geoArr = np.copy(geoArr[0:num-1]); geoArr = geoArr[idx]
    maj = np.copy(maj[1:num-1]); maj = maj[idx]
    tot = np.copy(tot[1:num-1]);tot=tot[idx]
    ...
    for i in range(num-2):
        A = A+((sample[i]*geoArr[i])/sum(sample))
        B = B+((maj[i]*geoArr[i])/sum(maj))
        totCum1 = totCum1 + tot[i]; totCum2 = totCum2+tot[num-2-i-1]
        if n1==0 and totCum1>=sum(sample):
            n1=i; fin1 = totCum1-tot[i]
        if n2==0 and totCum2>=sum(sample):
            n2=i; fin2 = totCum2-tot[num-2-i-1]
    for i in range(n1+1):
        C = C+((tot[i]*geoArr[i])/fin1)
    for i in range(n2,num-2):
        D = D+((tot[i]*geoArr[i])/fin2)
    print((C/D)-1)
    return(((A/B)-1)/((C/D)-1))
        
#--------------------------------------------------

segregation=np.zeros([4,5])
for i in range(4):
    if i==0:
        sample = asia
    elif i==1:
